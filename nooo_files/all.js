...
$.fn.uWorklet = function() {
	...
	var plugin = {		
		...
		{
			if(data.redirect) { window.location = data.redirect; delete data.redirect; }
...
(function($) {
	$.fn.uGoodsShippingUpdate = function()
	{
		var form = $(this);
		
		var toggleFields = function()
		{
			switch(form.find('select[name$="[regionType]"]').val())
			{
				case "1":
					form.find(".field_country").show();
					form.find(".field_state").hide();
					form.find(".field_city").hide();
					break;
				case "2":
					form.find(".field_country").show();
					if(form.find(".field_state").is(":visible"))
				...
				case "3":
					...
					if(form.find(".field_state").is(":visible"))
						form.find(".field_state").show();
					form.find(".field_city").show();
					break;
				default:
					form.find(".field_country").hide();
					form.find(".field_state").hide();
					form.find(".field_city").hide();
					break;
			}
		...
		form.find('select[name$="[regionType]"]').change(function(){
			form.find(':input[name$="[country]"]').change();
		});

        form.find(':input[name$="[country]"],select[name$="[state]"]').change(function(){			
			toggleFields();
			return true;
		});
...
(function($) {
	$.uniprogy.uGoodsPaymentCart = {
		init: function()
		{		
			if($('#items_goods_tax').length > 0)
			{
				var tax_row = $('#items_goods_tax').closest('tr');
				tax_row.find('.quantityField, .removeLink, .price').closest('td').html('');
				tax_row.find('.total').closest('td').html('<div class="total"></div>');
				tax_row.attr({id:'taxRow'});
			}

			if($('#items_goods_shipping').length > 0)
			{
				var shipping_row = $('#items_goods_shipping').closest('tr');
				shipping_row.find('.quantityField, .removeLink, .price').closest('td').html('');
				shipping_row.find('.total').closest('td').html('<div class="total"></div>');
				shipping_row.attr({id:'shippingRow'});
			}
			$(".quantityField").first().change();

			$(".goodsToHide").hide();
			$("#codMobile").val("123456");
			$("#codAddress").val("Goods");
			$("#badMobile").val("123456");
			$("#badAddress").val("Goods");
		},
		
		shipping: function(flag)
		{
			if(flag)
			{
				$('.paymentInformation').hide();
				$('#uForm_PaymentCheckout .buttons').hide();
			}
			else
			{
				$(".quantityField").first().change();
				$('.paymentInformation').show();
				$('#uForm_PaymentCheckout .buttons').show();
			}
		}
	}
})(jQuery);
...
$.uniprogy.uGift = {
	...
	saveGiftOptions: function(fromForm,toForm,formName,id,create)
	{
		$(fromForm+' :input[name^="'+formName+'"]').each(function(){
			if(!$(this).is(':radio') || $(this).is(':checked'))
				$.uniprogy.uGift.pushValue(toForm,$(this).attr('name')+'['+id+']',$(this).val(),create);
		});
	},
	
	loadGiftOptions: function(fromForm,toForm,formName,id,create)
	{
		$(fromForm+' :input[name^="'+formName+'"][name$="['+id+']"]').each(function(){
			if(!$(this).is(':radio') || $(this).is(':checked'))
			{
				var name = $(this).attr('name');
				name = name.substring(0,name.lastIndexOf('['));
				$.uniprogy.uGift.pushValue(toForm,name,$(this).val(),create);
			}
		});
	},
	
	pushValue: function(form,name,value,create)
	{
		if($(form + ' :input[name="' + name + '"]').length > 0)
			$.uniprogy.val(form + ' :input[name="' + name + '"]',value);
		else if(create)
			$('<input />').attr({'type': 'hidden', 'name': name, 'value': value, 'class': 'pushed'}).appendTo(form);
	},
	
	addGift: function(text,delLink,id)
	{
		var sel = '.gift-'+id;
		$(sel+' a.pushed').remove();
		$(sel+' a.delete').remove();
		$(sel+' a').hide();
		$(sel+' a').clone(true).addClass('pushed').html(text).appendTo(sel).after(' '+delLink).show();
		$(sel).addClass('isAGift');
		$(sel+' a.delete').click(function(e){
			e.preventDefault();
			$.ajax({
				url: $(this).attr('href'),
				data: 'id='+id,
	...
	removeGift: function(id)
	{
		var sel = '.gift-'+id;
		$(sel+' a.pushed').remove();
		$(sel).closest('form').find(':input[name$="['+id+']"].pushed').remove();
		$(sel+' a.delete').remove();
		$(sel).removeClass('isAGift');
		$(sel+' a').show();
	}
...
$.fn.uGiftCard = function() {
	var form = $(this);
	
	form.find(':input[name$="[to]"], :input[name$="[from]"], :input[name$="[amount]"], :input[name$="[message]"]').keyup(function(){
		updatePreview($(this));
		return true;
	});
...
(function($) {
	$.fn.uDealTwitter = function(options) {
		var that = this;
		$.getScript('http://widgets.twimg.com/j/2/widget.js', function() {
			...
			}).render().setUser(options.account).start();
		});
		return this;
	}
})(jQuery);
